...
public class DLBCodeBook implements CompressionCodeBookInterface {
  ...
  public void add(String str, boolean flushIfFull) {
    if (root == null) root = new DLBNode(str.charAt(0));

    boolean haveRoom = code < L; // setting haveRoom based on codeBook capacity

    // no room left
    if (!haveRoom) {
      // width less than maximum
      if (W < maxW) {
        W++; L = 1 << W;
        haveRoom = true;
      }
      // W >= maxW, and flushIfFull is true
      else if (flushIfFull) {
          initialize();
          haveRoom = true;
      }
